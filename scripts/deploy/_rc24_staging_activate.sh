#!/usr/bin/env bash
# RC2.4 / RC2.4.1 staging deploy — STAGING ONLY. Never touch production.
set -euo pipefail
cd /opt/omnia/platform
RC_SHA="${1:-393aabb42a933701ec93371b042ab1b9e55bad60}"
RC_BRANCH="${2:-release/rc24-tutor-quality-parity}"
STAMP="$(date -u +%Y%m%d-%H%M%S)"
BACKUP_DIR="/opt/omnia/backups/staging/rc241-${STAMP}"
mkdir -p "$BACKUP_DIR"
exec > >(tee -a "$BACKUP_DIR/REPORT.txt") 2>&1

echo "UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "RC24_SHA=$RC_SHA"
...
docker exec -e PGPASSWORD="$PASS" "$STAGING_DB_CONTAINER" \
  pg_dump -U "$DB_USER" -d "$DB_NAME" -Fc -f /tmp/omnia_staging_rc24.dump
docker cp "$STAGING_DB_CONTAINER:/tmp/omnia_staging_rc24.dump" "$BACKUP_DIR/omnia_staging.dump"
docker exec "$STAGING_DB_CONTAINER" rm -f /tmp/omnia_staging_rc24.dump
unset PASS
...
for i in $(seq 1 60); do
  if curl -fsS https://admin.dev.omniafrigo.com.br/api/health >/tmp/rc24_admin.json \
    && curl -fsS https://dev.omniafrigo.com.br/api/health >/tmp/rc24_web.json; then
    break
  fi
  sleep 8
done
cp /tmp/rc24_admin.json "$BACKUP_DIR/admin_health.json" 2>/dev/null || true
cp /tmp/rc24_web.json "$BACKUP_DIR/web_health.json" 2>/dev/null || true
python3 - <<PY
import json
from pathlib import Path
exp="$RC_SHA"
w=json.loads(Path("/tmp/rc24_web.json").read_text())
a=json.loads(Path("/tmp/rc24_admin.json").read_text())
...
# Ensure runtime GIT_SHA override if build ARG missed (same pattern as RC2.3)
cat > /tmp/rc24_gitsha_override.yml <<YAML
services:
  admin:
    environment:
      GIT_SHA: "$RC_SHA"
...
GIT_SHA="$RC_SHA" APP_VERSION="$RC_SHA" \
  docker compose -f docker/compose/staging.yml --env-file .env.staging \
  -f /tmp/rc24_gitsha_override.yml \
...
echo "RC24_STAGING_ACTIVATE_DONE"
