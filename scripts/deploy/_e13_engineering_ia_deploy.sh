#!/usr/bin/env bash
# Epic 13 Engenharia IA — staging only
set -euo pipefail
cd /opt/omnia/platform

STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="/opt/omnia/backups/staging/engineering-ia-e13-${STAMP}"
...
python3 - <<'PY' "$DATABASE_URL" > /tmp/_e13_db_meta.env
...
source /tmp/_e13_db_meta.env
rm -f /tmp/_e13_db_meta.env
...
docker exec -e PGPASSWORD="$PGPASSWORD" "$PG_CONT" \
  pg_dump -U "$DB_USER" -d "$DB_NAME" -Fc -f "/tmp/omnia_staging_e13_${STAMP}.dump"
docker cp "$PG_CONT:/tmp/omnia_staging_e13_${STAMP}.dump" "$BACKUP_DIR/omnia_staging.dump"
docker exec "$PG_CONT" rm -f "/tmp/omnia_staging_e13_${STAMP}.dump"
unset PGPASSWORD
{
  echo "stamp=$STAMP"
  echo "prev_head=$PREV"
...
docker compose -f docker/compose/staging.yml --env-file .env.staging \
  ...
  'cd /app && /usr/local/bin/omnia-admin-bootstrap.sh engineering-ia-epic13'
...
docker ps --filter name=omnia-platform-web-prod --format 'web-prod={{.Status}}'
docker ps --filter name=omnia-lms-moodle-dev --format 'moodle={{.Status}}'
echo "E13_DEPLOY_OK HEAD=$NEW_HEAD BACKUP_DIR=$BACKUP_DIR"
