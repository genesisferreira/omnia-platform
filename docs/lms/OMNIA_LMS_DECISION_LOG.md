...
| D020 | 2026-07-31 | **Neurofrigo Runtime Spec** — pipeline obrigatório Identity→Intent→Security→Context→ToolRouter→Specialist→Compliance; Assessment Integrity; ACL-first RAG; sem implementação de LLM nesta sprint; Write acadêmico via IA só após Write APIs | Sprint 3.1 | Implementação NF Fase 1 após Write APIs (recomendado) | **Aprovado (spec)** |
| D021 | 2026-08-03 | **Neurofrigo Runtime Spec v1.1** — Purpose Guard antes do Intent (Omnia-only; categorias + recusa padrão); Tutor/Concierge ≠ Command (`super_admin` / `command.allowedRoles`); MVP = chat Portal apenas; WhatsApp fora do MVP; n8n ≠ cérebro/canal; DeepSeek = provider inicial via adapter desacoplado (sem SDK/integração nesta etapa) | Adendo NF | Pipeline Identity→Purpose→Intent→…; Fase 1 Portal; Command pós-Portal; WhatsApp/DeepSeek wired só com GO | **Aprovado (spec)** |
| D022 | 2026-08-03 | **Neurofrigo Knowledge Hub Foundation (Macroentrega 01)** — Admin Payload (documents/categories/sources/reviews/agent-access/jobs/audit/settings); workflow + ACL; ingestão manual; sensíveis = draft + INTERNAL_RESTRICTED + high risk + allowAiUse false; sem embeddings/LLM/WhatsApp/ERP; staging homolog; prod + landing intactas | Fundação KB | RAG/indexação pós-aprovação em ME futura; Runtime chat após Hub | **Aprovado (fundação)** |
